...
describe('fs URI encoding support', (report, done) => {
  ...
  .then((stat) => {
    report(
      <Assert key="append content from URI should be correct" expect={sizes[0]+sizes[1]} actual={Math.floor(stat.size)}
      />)
  ...
  .then((stat) => {
    report(
      <Assert key="replace content from URI should be correct" expect={sizes[1]} actual={Math.floor(stat.size)}
      />)
    done()
  })
  .catch((err) => {
...
describe('request timeout working properly', (report, done) => {
  ...
  .then(() => {
    report(<Assert key="should not execute successfully" expect={true} actual={false}/>)
    done()
  })
  .catch((err) => {
    report(<Assert key="expect timeout error" expect={true} actual={/timed out/ig.test(err)}/>)
    done()
  })
})
...
describe('regular request should have correct body', (report, done) => {
  ...
  .then((res) => {
    report(
      <Assert key="check headers" expect={'bar'} actual={res.json().headers.foo}/>,
      <Assert key="check content type" expect={'text/foo'} actual={res.json().headers['content-type']}/>,
      <Assert key="check body" expect={'foo is bar'} actual={res.json().body}/>
    )
    done()
  })
  .catch((err) => {
...
describe('automatic content conversion test', (report, done) => {
  ...
  .then((res) => {
    report(<Assert key="request body should be decoded by BASE64 decoder" expect={expect1} actual={res.json().body}/>)
  ...
  .then((resp) => {
    report(<Assert key="request body should send from storage" expect={expect2} actual={resp.json().body}/>)
    done()
  })
  .catch((err) => {
