...
describe('cancel task should work properly', (report, done) => {
  ...
  Timer.setTimeout(() => {
    ...
    .then(() => done())
    .catch((err) => {
      report(<Assert key="unlink should not have failed" expect={null} actual={err}/>)
      done()
    })
  }, 10000)
  ...
  task
  .then((res) => {
    report(<Assert key="task not canceled" expected={false} actual={true}/>)
  })
  .catch((resp) => {
    report(<Assert key="task cancelled rejection should be catchable" expect={true} actual={true}/>)
    done()
  })
})
