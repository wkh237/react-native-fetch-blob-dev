...
describe('#227 IOS file modification date correctness', (report, done) => {
  ...
  .then((stat) => {
    ...
    report(<Assert key="modification date should be correct" expect={true} actual={correct}/>);
    done()
  })
  .catch((err) => {
...
describe('#241 null header silent failed issue', (report, done) => {
  ...
  .then(() => {
    report(<Assert key="null header should not crash the app" expect={true} actual={true}/>)
    done()
  })
  .catch((err) => {
...
describe('#248 create blob from file has spaces in filename', (report, done) => {
  ...
  .then((stat) => {
    report(<Assert key="blob created without error" expect={stat.size} actual={size}/>)
