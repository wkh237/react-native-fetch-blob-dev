...
describe('GET request with params', (report, done) => {
  ...
  .then((stream) => {
    ...
    stream.onEnd(() => {
      ...
      report(
        <Assert key="param#1 should be correct" expect={parseInt(time)} actual={parseInt(result.time)}/>,
        <Assert key="param#2 should be correct" expect={'RNFetchBlobParams'} actual={result.name}/>,
        <Assert key="param contains unicode data should be correct" expect={'中文'} actual={result.lang}/>
      )
      done()
    })
  })
...
describe('POST request with params', (report, done) => {
  ...
  .then((stream) => {
    ...
    stream.onEnd(() => {
      ...
      result = JSON.parse(result)
      report(
        <Assert key="param#1 should be correct"
                expect={parseInt(time)}
                actual={parseInt(result.time)}/>,
        <Assert key="param#2 should be correct"
                expect={'RNFetchBlobParams'}
                actual={result.name}/>,
        <Assert key="param contains unicode data should be correct"
                expect={'中文'}
                actual={result.lang}/>
      )
      done()
