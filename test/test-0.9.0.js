...
describe('#73 unicode response BASE64 content test', (report, done) => {
  ...
  .then((data) => {
    report(<Assert key="data should be correct" expect={'你好!'} actual={data.data}/>)
    done()
  })
  .catch((err) => {
...
describe('request should not retry after being timed out', (report, done) => {
  let count = 0

  let task = RNFetchBlob.fetch('GET', `${TEST_SERVER_URL}/timeout408/${Date.now()}`)

  task.then((res) => {
    report(<Assert key="request should not have succeeded" expect={true} actual={false}/>)
  })
  .catch(() => {
    task.cancel()
    count++
