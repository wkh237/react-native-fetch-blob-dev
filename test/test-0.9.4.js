...
describe('chunked encoding option test', (report, done) => {
  ...
  .then((res) => {
    ...
    report(<Assert key="request should not use chunked encoding" expect={undefined} actual={headers['transfer-encoding']}/>)
    done()
  })
  .catch((err) => {
...
describe('#118 readStream performance prepare the file', (report, done) => {
  ...
  let task = RNFetchBlob.config({fileCache: true}).fetch('GET', `${TEST_SERVER_URL}/public/22mb-dummy`)

  task.progress((current, total) => {
    report(
      <Info key="prepare file" uid="prepare">
